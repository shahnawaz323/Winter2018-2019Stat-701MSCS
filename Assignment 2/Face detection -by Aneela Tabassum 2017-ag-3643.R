...
# Automatically Installing packages for 3D-graph
...
Y = data([1,2,3,4,5,6])
X1 = data([0,0,10,10,20,20])
X2 = data([10,12,15,25,20,30])
...
# Code for the 3D-plot and Regression line, 
data(df)
head(df)
#scatterplot3d(x=X1, y=X2, z=Y)
s3d <- scatterplot3d(df, type = "h", color = "blue",
    angle=45, pch = 16,grid=TRUE, box=FALSE,


              main="Multiple Linear Regression \n By Aneela Tabassum \n 2017-ag-3643",
              xlab = "no. features detected on the face ",
              ylab = " power of GPU ",
              zlab = " processing time ")
...
# Combines the Data-set of X1 , Rainfall and Y

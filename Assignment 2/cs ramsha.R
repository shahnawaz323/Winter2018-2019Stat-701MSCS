...
rm(list=ls(all=TRUE))
picture.Graphics = c(15,17,115,117,19,111)
Color.Combinations = c(10,10,110,110,120,120)
Axis.Depth= c(10,10,1100,1100,1400,1400)
df = data.frame(picture.Graphics,Color.Combinations,Axis.Depth)
data(df)
head(df)
scatterplot3d(x=picture.Graphics, y=Color.Combinations, z=Axis.Depth)
...
my.lm <- lm(picture.Graphics~Color.Combinations+Axis.Depth)
...
Exp.data = cbind(picture.Graphics, Color.Combinations, Axis.Depth)
colnames(Exp.data) = c('picture.Graphics', 'Color.Combinations', 'Axis.Depth')
Exp.data

# let's conduct simple linear regression on data by calling our function
Exp.regression <- MLR(picture.Graphics, Color.Combinations, Axis.Depth)
Exp.regression
